...
export default function DashboardClient() {
  ...
  const loadPlants = useCallback(async () => {
    ...
    try {
      ...
      setError(loadError instanceof Error ? loadError.message : "Unable to load plants");
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  const updatePlant = async (
    plantId: number,
    updates: Partial<Pick<Plant, "watering_duration" | "watering_frequency" | "enabled">>
  ) => {
    if (!token) {
      return;
    }
    ...
    try {
      ...
      setError(updateError instanceof Error ? updateError.message : "Failed to update plant");
    } finally {
      setUpdatingId(null);
    }
  };
    ...
    <div className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_15%_20%,rgba(0,255,170,0.09),transparent_35%),radial-gradient(circle_at_85%_5%,rgba(89,129,255,0.1),transparent_30%),linear-gradient(180deg,#050505_0%,#090909_45%,#030303_100%)] text-zinc-100">
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8 sm:px-8 lg:py-12">
        <header className="rounded-3xl border border-white/10 bg-black/45 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.04),0_20px_70px_rgba(0,0,0,0.65)] backdrop-blur-lg">
          <p className="text-xs uppercase tracking-[0.28em] text-emerald-300/85">Smart Irrigation</p>
          <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl font-semibold text-white sm:text-4xl">Watering Control Dashboard</h1>
              <p className="mt-2 max-w-2xl text-sm text-zinc-300">
                Monitor all plants, create new watering schedules, and toggle devices in real-time.
              </p>
            </div>
            <button
              type="button"
        ...
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <MetricCard
            ...
            value={
              plants.length > 0
                ? `${Math.round(
                    plants.reduce((sum, plant) => sum + plant.watering_frequency, 0) / plants.length
                  )}h`
                : "-"
            }
          />
        ...
        {!token && (
          <AlertCard tone="error" message="Missing token. Open this dashboard with ?token=YOUR_DEVICE_TOKEN" />
        )}
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-[1.1fr_1.9fr]">
          ...
          >
            ...
            <p className="mt-1 text-sm text-zinc-400">Add a new schedule to your watering fleet.</p>

            <label className="mt-5 block text-xs uppercase tracking-[0.16em] text-zinc-400" htmlFor="duration">
          ...
          <div className="rounded-2xl border border-white/10 bg-zinc-950/80 p-5 shadow-lg shadow-black/30">
            ...
            <p className="mt-1 text-sm text-zinc-400">Update duration, frequency, and status per plant.</p>
            ...
            <div className="mt-4 overflow-hidden rounded-xl border border-white/10">
              ...
              {plants.length === 0 ? (
                <div className="px-3 py-6 text-center text-sm text-zinc-500">
                  {loading ? "Loading plants..." : "No plants yet. Create one to get started."}
                </div>
              ) : (
...
type PlantRowProps = {
  plant: Plant;
  disabled: boolean;
  onSave: (updates: Partial<Pick<Plant, "watering_duration" | "watering_frequency" | "enabled">>) => void;
};
